...
auto it = db->NewIterator();                // merge iterator across all levels
...
```
Put("k","v") ──→ WAL (CRC32) ──→ MemTable (lock-free skip-list)
                                       │
...
|---|---|---|
| **WAL** | ~40 | Append-only write-ahead log with per-record CRC32, timestamp, and type. Truncation-tolerant decoder — never returns torn records. |
...
| **Engine** | ~250 | Flush and compaction workers (POSIX threads or inline for WASM). Readers acquire `shared_lock` only — writers never stall I/O. Leveled compaction at 10× size ratio per layer. |
| **Recovery** | ~56 | Manifest-based replay: atomically swap `MANIFEST` via `rename()`. Replays only manifest-referenced WALs; orphans are pruned. |

### Concurrency

- `std::shared_mutex` — writers hold exclusive, readers hold shared
- Background flush and compaction release the mutex during I/O
- No background threads in WASM build — `MaintainInline` runs maintenance synchronously
- `Iterator` provides snapshot isolation via MemTable pointer capture
...
```sh
scripts/bench.sh                           # release build + all benchmarks
build-release/tinylsm_bench                # raw benchmark driver
scripts/compare.sh     # head-to-head with LevelDB
node scripts/spec_check.mjs                # validate against design targets
```

### Concurrent write throughput

tinylsm drives **327,389 ops/s** across 4 client threads. LevelDB reaches 421,286 ops/s on the same hardware (14-Core Apple M4 Max, 64 GB, macOS 15, APFS). tinylsm's throughput is constrained by per-put exclusive locking — a known narrowing opportunity.
...
│   ├── engine.hpp              # Engine class declaration
...
│   ├── workers.cpp             # Write, Get, Flush, Compact, Merge, iterators, stats
│   └── recovery.cpp            # Manifest, Load, Init, destructor
├── app/                        # CLI REPL
├── bench/                      # Microbenchmarks + LevelDB comparison
├── tests/                      # 8 test binaries + 2 fuzz targets
├── scripts/                    # Build, CI, benchmark, coverage tooling
├── reports/                    # benchmark.jsonl, competitors.jsonl, coverage.txt
